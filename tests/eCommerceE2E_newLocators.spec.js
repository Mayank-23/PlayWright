...
test('E2E Journey of eCommerce', async ({browser})=>{
    ...
    await cardTitles.filter({hasText: 'ZARA COAT 3'}).getByRole('button',{name:"Add to Cart"}).click();

    // for loop not required as there is playwright specific locators present to sort out the things
